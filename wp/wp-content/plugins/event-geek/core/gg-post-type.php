...
function gg_events_posttype() {
	$rewrites = array(
	 	'slug' => apply_filters('gg_events_slug', __('events', 'event_geek'))
	);	
    ...
    register_post_type( 'gg_events',
        array(
            ...
            'public' => apply_filters('gg_events_public', false),
			'publicly_queryable' => true,
			'show_ui' => true,
			'has_archive' => apply_filters('gg_events_archive', false),
            'supports' => array('title', 'editor', apply_filters('gg_event_supports','revisions')),
			'rewrite' => $rewrites,
            'capability_type' => 'post',
            'menu_position' => 5,
            'menu_icon' => apply_filters('gg_events_menu_icon', plugins_url() . '/event-geek/images/calendar-icon.png'),
...
//delare meta variable names
	...
	if(!$gg_event_options['customize_event_info']){ //if info customization is not turned on
		...
		foreach($saved_inputs as $input){
			...
			if($saved_types[$count] == 'link'){$names[] = $name . '_link_text';}
	...
	$names = apply_filters('gg_event_meta_names', $names);
...
if($gg_event_options['customize_event_info']){ //if info customization is turned on
	...
	$count = 0;
		...
		<?php
			if($saved_types[$count]=='link'){ ?>
            <p><?php echo $input . ' '; _e('Link Text', 'event_geek'); ?>: <input name="<?php echo $name; ?>_link_text" type="text" value="<?php echo $meta[$name . '_link_text']; ?>" /></p>
             <?php }
		$count++;
		 }//foreach
...
<?php 
	do_action('gg_event_meta_inputs', $meta);
} //gg_events_options
...
function gg_save_events_meta($post_id, $post) {
    ...
    if ( !wp_verify_nonce( $_POST['event_meta_noncename'], plugin_basename(__FILE__) )) {
		do_action('gg_failed_nonce');
	    return $post->ID;
    }
 
    // Is the user allowed to edit the post or page?
    if ( !current_user_can( 'edit_post', $post->ID )){
		do_action('gg_failed_permissions');
        return $post->ID;
	}
	...
	$events_meta = gg_get_posted_meta();
		...
		if($events_meta['gg_is_single_day']){ 
			$events_meta['gg_event_dates'] = date($gg_event_options['gg_event_date_format'], strtotime($events_meta['gg_event_date_start_standard_format']));
			$events_meta['gg_event_date_end_standard_format'] = $events_meta['gg_event_date_start_standard_format'];
		} else{
			$startDate = strtotime($events_meta['gg_event_date_start_standard_format']);
			$endDate = strtotime($events_meta['gg_event_date_end_standard_format']);
	...
	do_action('gg_save_events_meta', $events_meta, $post_id, $post);
}//gg_save_events_meta
 
add_action('save_post', 'gg_save_events_meta', 1, 2); // save the gg fields

// Register Custom Taxonomy
function gg_event_taxonomy()  {

	$labels = array(
		'name'                       => _x( 'Event Categories', 'Taxonomy General Name', 'event_geek' ),
		'singular_name'              => _x( 'Event Category', 'Taxonomy Singular Name', 'event_geek' ),
		'menu_name'                  => __( 'Event Categories', 'event_geek' ),
		'all_items'                  => __( 'All Categories', 'event_geek' ),
		'parent_item'                => __( 'Parent Category', 'event_geek' ),
		'parent_item_colon'          => __( 'Parent Category:', 'event_geek' ),
		'new_item_name'              => __( 'New Category', 'event_geek' ),
		'add_new_item'               => __( 'Add New Category', 'event_geek' ),
		'edit_item'                  => __( 'Edit Category', 'event_geek' ),
		'update_item'                => __( 'Update Category', 'event_geek' ),
		'separate_items_with_commas' => __( 'Separate Categories with commas', 'event_geek' ),
		'search_items'               => __( 'Search categories', 'event_geek' ),
		'add_or_remove_items'        => __( 'Add or remove categories', 'event_geek' ),
		'choose_from_most_used'      => __( 'Choose from the most used categories', 'event_geek' ),
	);
	$args = array(
		'labels'                     => $labels,
		'hierarchical'               => true,
	...
	register_taxonomy( 'event_category', 'gg_events', apply_filters('gg_event_taxonomy_args', $args) );

}

// Hook into the 'init' action
add_action( 'init', 'gg_event_taxonomy', 0 );


// Add start date column to admin event list
function gg_event_geek_column_head($defaults) {  
	$postype = $_GET['post_type'];
	if($postype == 'gg_events'){
		$defaults['gg_start_date'] = 'Start Date';  
		$defaults['gg_end_date'] = 'End Date';  
	}
    return $defaults;  
}  
add_filter('manage_posts_columns', 'gg_event_geek_column_head'); 
  
function gg_event_geek_columns_content($column_name, $post_ID) {  
	$postype = $_GET['post_type'];
	if($postype == 'gg_events'){
        $gg_names = get_post_meta($post_ID, 'gg_names', true); //get list of meta names
        $meta = gg_get_saved_meta($gg_names,$post_ID);// load meta data	

		if ($column_name == 'gg_start_date') {  
			echo date_i18n(get_option('date_format'), strtotime($meta['gg_event_date_start_standard_format']));
		}  			

		if ($column_name == 'gg_end_date') {  
			echo date_i18n(get_option('date_format'), strtotime($meta['gg_event_date_end_standard_format']));
		}  			

	}//if($postype == 'gg_events')

}  
add_filter( 'manage_posts_custom_column', 'gg_event_geek_columns_content', 10, 3 );
?>